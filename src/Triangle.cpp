...
namespace EE {
    Triangle::Triangle() {
        this->vertices[0] = glm::vec3(0.f, 0.f, 0.f);
        this->vertices[1] = glm::vec3(0.f, 0.f, 0.f);
        this->vertices[2] = glm::vec3(0.f, 0.f, 0.f);
    ...
    std::array<glm::vec4, 3> Triangle::ToVector4() {
        ...
        std::transform(std::begin(vertices), std::end(vertices), res.begin(), [](auto& vec) { return glm::vec4(vec.x, vec.y, vec.z, 1.f); });
        return res;
    }
} // EE